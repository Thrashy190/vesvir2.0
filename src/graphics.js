...
export default class GraphicsEngine {
   ...
   */
  constructor(_canvas, _joints, _model) {
    this.canvas = _canvas;
  ...
  initScene() {
    ...
    this.torsoSphere = BABYLON.MeshBuilder.CreateSphere(
      'torsoSphere',
      { diameter: 0.5 },
      this.scene
    );
    this.importMesh('./assets/', 'camisaG.glb');
  }

  importMesh(url, fileName, scene = this.scene) {
  ...
  updateMesh(newMeshes, particleSystems, skeletons) {
    ...
    this.scene.registerBeforeRender(() => {
      ...
      this.sphere.position.x = 0 + data.head.x;
      this.sphere.position.y = 6 + data.head.y;
      this.sphere.position.z = 5;

      /** Code to move back and forth torsoSphere */
      t += 0.02;
      this.torsoSphere.position.x = 2 * Math.sin(t);

      /** torsoSphere controls where the torso bone looks at */
      //torsoSphere.position.x = 0 + data.torso.x;
      this.torsoSphere.position.y = 5;
      this.torsoSphere.position.z = -5;
      ...
      left_arm_bone.rotation = new BABYLON.Vector3(data.leftElbow * 0.7, 0, 0);
    });
  }
